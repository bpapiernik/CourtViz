...
const RollingFGChart = dynamic(() => import('../../components/RollingFGChart'), { ssr: false });
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '28px 20px' }}>
        ...
        {playerInfo && (
          <div className="fade-up" style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              ...
              >← Players</Link>
              <img src={headshotUrl || '/default-headshot.png'} alt={playerInfo.player_name}
                style={{ width: 88, height: 88, borderRadius: '50%', objectFit: 'cover', border: `3px solid ${posColor}` }} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                <h2 style={{ fontSize: 24, fontWeight: 700, margin: 0, letterSpacing: -0.5 }}>{playerInfo.player_name}</h2>
                <span style={{
                  background: `${posColor}22`, color: posColor, border: `1px solid ${posColor}55`,
                  borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700,
                  fontFamily: 'var(--font-mono)', letterSpacing: 1,
                }}>{playerInfo.position}</span>
              </div>
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={label}>
                ...
                <select className="ctrl-select" value={value}
                  onChange={e => setter(e.target.value)} style={selectStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {[
            ...
            <div key={label} style={{ ...sectionCardStyle, borderTop: `3px solid ${posColor}` }}>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{label}</div>
                {sublabel && <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', opacity: 0.45 }}>{sublabel}</div>}
              </div>
              <select className="ctrl-select" value={group}
                onChange={e => setGroup(e.target.value)}
                style={{ ...selectStyle, width: '100%', marginBottom: 12 }}>
                {Object.keys(statGroups).map(g => <option key={g} value={g}>{g}</option>)}
              </select>
