...
const createArcTrace = (xCenter, yCenter, width, height, theta1, theta2, dashed = false, color = '#94a3b8', lw = 1.5) => {
  ...
  return {
    x, y,
    mode: 'lines',
    line: { color, width: lw, dash: dashed ? 'dash' : 'solid' },
    hoverinfo: 'skip',
    showlegend: false,
    type: 'scatter',
  };
...
const generateCourtShapesAndArcs = (lw = 1.5, color = '#94a3b8') => {
  const shapes = [
...
const ShotChart = ({ bins, playerName }) => {
  ...
  // Filter out near-zero frequency bins entirely
  const visible = bins.filter(d => d.FREQ_PCT >= 0.0015);
 
  const x        = visible.map(d => d.HEX_X * 5);
  const y        = visible.map(d => d.HEX_Y * 5);
  const color    = visible.map(d => Math.max(-0.25, Math.min(0.25, d.FG_PCT_DIFF)) * 100);
  const freq     = visible.map(d => d.FREQ_PCT);
  const maxFreq  = Math.max(...freq);
  const sizes    = freq.map(f => Math.sqrt(f / maxFreq) * 40);
  ...
  return (
    <Plot
      ...
      layout={{
        ...
        plot_bgcolor: 'rgba(0,0,0,0)',  // transparent — matches your cream bg
        paper_bgcolor: 'rgba(0,0,0,0)', // transparent
        margin:      { t: 10, b: 0, l: 0, r: 50 },
        height:      500,
        shapes,
        showlegend:  false,
        autosize:    true,
